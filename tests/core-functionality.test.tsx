...
describe('React Slot System - Core Functionality', () => {
  ...
  describe('hasSlots HOC', () => {
    it('should enhance component with slot functionality', () => {
      const BaseComponent = ({ title }: { title: string }) => (
        <div>
          <h1>{title}</h1>
          <Slot name="content" />
        </div>
  ...
  describe('Slot rendering', () => {
    ...
    it('should render template content when provided', () => {
      ...
      expect(screen.getByText('Template Header')).toBeDefined();
  ...
  describe('Complex scenarios', () => {
    ...
    it('should handle conditional slot rendering', () => {
      const Component = hasSlots(({ showSlot, children }: PropsWithChildren<{ showSlot: boolean }>) => (
        <div>
      ...
      expect(screen.getByText('Conditional content')).toBeDefined();
    });
  ...
  describe('Edge cases', () => {
    it('should handle empty slot names', () => {
      ...
      render(<Component />);
      expect(screen.getByText('Empty name')).toBeDefined();
    });
    ...
    it('should work with components that have no templates', () => {
      ...
      render(<Component />);
      expect(screen.getByText('No slots here')).toBeDefined();
    });
  });
});